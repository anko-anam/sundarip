...
add_action( 'customize_controls_enqueue_scripts',array('Shopcozi_Editor_Scripts','enqueue'),95);

function shopcozi_customizer_control_scripts(){
    wp_enqueue_media();
    wp_enqueue_script('jquery-ui-sortable');
    wp_enqueue_script('wp-color-picker');
    wp_enqueue_style('wp-color-picker');
    wp_enqueue_script('shopcozi-customizer',get_template_directory_uri().'/inc/customizer/js/customizer.js',array('customize-controls','wp-color-picker'));
    wp_enqueue_style('shopcozi-customizer-js',get_template_directory_uri().'/inc/customizer/css/customizer.css');
}
add_action( 'customize_controls_enqueue_scripts','shopcozi_customizer_control_scripts',99);

function shopcozi_enqueue_editor(){
    if( ! isset( $GLOBALS['__wp_mce_editor__'] ) || ! $GLOBALS['__wp_mce_editor__'] ) {
        $GLOBALS['__wp_mce_editor__'] = true;
        ?>
        <script id="_wp-mce-editor-tpl" type="text/html">
...
function shopcozi_customize_preview_script() {
    wp_enqueue_script(
        'shopcozi-customize-preview',
...
add_action( 'customize_preview_init','shopcozi_customize_preview_script');

function shopcozi_customize_controls_icons_scripts(){
    $icons = shopcozi_fontawesome_icons_list();
    
    $string_icons='';
    foreach( $icons as $key => $value ){
        $string_icons = $string_icons.'|'.$key;
    }
    wp_localize_script( 'customize-controls', 'shopcozi_icons', array_keys($icons) );
    ...
    wp_localize_script( 'customize-controls', 'C_Icon_Picker',
        apply_filters( 'c_icon_picker_js_setup',
            array(
                'search'    => esc_html__( 'Search', 'shopcozi' ),
                'fonts' => array(
                    'font-awesome' => array(
                        // Name of icon
                        'name' => esc_html__( 'Font Awesome', 'shopcozi' ),
...
add_action( 'customize_controls_enqueue_scripts','shopcozi_customize_controls_icons_scripts');

function shopcozi_fontawesome_icons_list(){
    return array (
        'fab fa-500px' => '500px',
